package DataBooks;

import java.util.HashMap;

public class ListBooks {

    HashMap<String, Object> t = new HashMap<>();

    public ListBooks() {

        Love_1 book36 = new Love_1("Câu chuyện tình yêu đầy cảm xúc và sự hy sinh.", "Sách L", "Tác giả A", "NXB A", 2023, 200, "Một câu chuyện tình yêu đáng đọc", 25);
        Love_1 book37 = new Love_1("Tình yêu giữa hai người từ hai thế giới khác nhau.", "Sách B", "Tác giả B", "NXB B", 2021, 180, "Một câu chuyện tình yêu thú vị", 22);
        Love_1 book38 = new Love_1("Một cuộc tình ngọt ngào bắt đầu từ một lời hứa.", "Sách C", "Tác giả C", "NXB C", 2019, 250, "Một câu chuyện tình yêu ngọt ngào", 28);
        Love_1 book39 = new Love_1("Tình yêu đích thực luôn luôn tìm đường trở về.", "Sách D", "Tác giả D", "NXB D", 2017, 220, "Một câu chuyện tình yêu đích thực", 30);
        Love_1 book40 = new Love_1("Nụ hôn đầu tiên trong ngày nắng.", "Sách E", "Tác giả E", "NXB E", 2015, 270, "Một câu chuyện tình yêu lãng mạn", 26);
        Love_1 book41 = new Love_1("Cảm giác yêu thương khi người kia cười.", "Sách F", "Tác giả F", "NXB F", 2013, 230, "Một câu chuyện tình yêu cảm động", 24);
        Love_1 book42 = new Love_1("Một mối tình qua thư từ ngọt ngào.", "Sách G", "Tác giả G", "NXB G", 2011, 300, "Một câu chuyện tình yêu qua thư từ", 32);
        Love_1 book43 = new Love_1("Hai trái tim hòa quyện trong một bữa tiệc đầy lãng mạn.", "Sách H", "Tác giả H", "NXB H", 2010, 280, "Một câu chuyện tình yêu lãng mạn", 29);
        Love_1 book44 = new Love_1("Bước chân ngại ngùng trên con đường tình yêu.", "Sách I", "Tác giả I", "NXB I", 2018, 260, "Một câu chuyện tình yêu thú vị", 27);
        Love_1 book45 = new Love_1("Tình yêu thời chiến đấu vì hòa bình.", "Sách J", "Tác giả J", "NXB J", 2016, 240, "Một câu chuyện tình yêu lịch sử", 25);
        Love_1 book46 = new Love_1("Những lời thổ lộ trong ánh đèn đường.", "Sách K", "Tác giả K", "NXB K", 2014, 210, "Một câu chuyện tình yêu nhẹ nhàng", 23);
        Love_1 book47 = new Love_1("Suy nghĩ về người yêu dấu khi rơi nước mắt.", "Sách L", "Tác giả L", "NXB L", 2012, 290, "Một câu chuyện tình yêu cảm động", 28);
        Love_1 book48 = new Love_1("Chạm vào trái tim qua một bài thơ.", "Sách M", "Tác giả M", "NXB M", 2011, 270, "Một câu chuyện tình yêu lãng mạn", 26);
        Love_1 book49 = new Love_1("Những đôi mắt nhìn nhau giữa biển người.", "Sách N", "Tác giả N", "NXB N", 2019, 310, "Một câu chuyện tình yêu cảm động", 30);
        Love_1 book50 = new Love_1("Bàn tay nắm chặt nhau không rời.", "Sách O", "Tác giả O", "NXB O", 2020, 320, "Một câu chuyện tình yêu chân thành", 32);

        horrified_2 book1 = new horrified_2("Không gian sợ hãi", "Tâm lý đáng sợ", "Sách Kinh dị A", "Tác giả A", "NXB X", 2023, 250, "Mô tả sự kinh dị và tâm lý của con người", 28);
        horrified_2 book2 = new horrified_2("Ám ảnh của quá khứ", "Sự phân tâm của tâm trí", "Sách Kinh dị B", "Tác giả B", "NXB Y", 2021, 220, "Câu chuyện kinh dị và tâm lý đáng sợ", 26);
        horrified_2 book3 = new horrified_2("Cảm giác sợ hãi mỗi đêm", "Nỗi sợ hãi bất chợt", "Sách Kinh dị C", "Tác giả C", "NXB Z", 2019, 280, "Câu chuyện ám ảnh và tâm lý", 30);
        horrified_2 book4 = new horrified_2("Kinh dị từ sâu thẳm của tâm trí", "Mối liên kết tâm lý kinh hoàng", "Sách Kinh dị D", "Tác giả D", "NXB XYZ", 2017, 300, "Mô tả kinh dị và tâm lý của con người", 32);
        horrified_2 book5 = new horrified_2("Nỗi sợ hãi của một đời", "Ám ảnh từ những nỗi sợ hãi", "Sách Kinh dị E", "Tác giả E", "NXB Q", 2015, 260, "Câu chuyện kinh dị và tâm lý phức tạp", 29);
        horrified_2 book6 = new horrified_2("Mất ngủ vì ám ảnh từ kinh dị", "Sự sợ hãi không lối thoát", "Sách Kinh dị F", "Tác giả F", "NXB W", 2013, 240, "Mô tả cảm giác kinh dị và sự ám ảnh", 27);
        horrified_2 book7 = new horrified_2("Kinh dị từ những sự kiện thật sự", "Tâm lý sợ hãi chân thật", "Sách Kinh dị G", "Tác giả G", "NXB E", 2011, 280, "Một cuộc hành trình qua sự kinh dị và tâm lý", 31);
        horrified_2 book8 = new horrified_2("Ám ảnh từ sự sống và cái chết", "Mối đe dọa tâm lý", "Sách Kinh dị H", "Tác giả H", "NXB R", 2019, 270, "Câu chuyện về sự sống và cái chết", 28);
        horrified_2 book9 = new horrified_2("Cảm giác sợ hãi từ những người lạ", "Ám ảnh từ nỗi sợ hãi", "Sách Kinh dị I", "Tác giả I", "NXB T", 2018, 290, "Mô tả cảm giác sợ hãi và sự ám ảnh", 30);
        horrified_2 book10 = new horrified_2("Sự kinh dị của tâm trí con người", "Nỗi sợ hãi của những nỗi ám ảnh", "Sách Kinh dị J", "Tác giả J", "NXB U", 2016, 310, "Câu chuyện kinh dị và tâm lý phức tạp", 32);
        horrified_2 book11 = new horrified_2("Tâm lý sợ hãi từ những kinh dị", "Ám ảnh từ những nỗi sợ hãi", "Sách Kinh dị K", "Tác giả K", "NXB I", 2014, 280, "Mô tả cảm giác sợ hãi và sự ám ảnh", 30);
        horrified_2 book12 = new horrified_2("Mối liên kết tâm lý giữa kinh dị", "Nỗi sợ hãi của tâm trí", "Sách Kinh dị L", "Tác giả L", "NXB O", 2012, 300, "Câu chuyện về mối liên kết tâm lý và sự kinh dị", 31);
        horrified_2 book13 = new horrified_2("Mối đe dọa tâm lý kinh dị", "Nỗi sợ hãi từ sự kinh dị", "Sách Kinh dị M", "Tác giả M", "NXB P", 2010, 270, "Mô tả cảm giác sợ hãi và sự ám ảnh từ kinh dị", 28);
        horrified_2 book14 = new horrified_2("Ám ảnh từ sự hồi tưởng kinh dị", "Nỗi sợ hãi từ những kinh dị", "Sách Kinh dị N", "Tác giả N", "NXB J", 2018, 260, "Câu chuyện về sự hồi tưởng và sự kinh dị", 27);
        horrified_2 book15 = new horrified_2("Sự kinh dị từ những thái độ tâm lý", "Nỗi sợ hãi từ những nỗi kinh dị", "Sách Kinh dị O", "Tác giả O", "NXB Z", 2020, 280, "Mô tả cảm giác sợ hãi và sự kinh dị", 29);

        Education_3 book16 = new Education_3("Toán học căn bản", "Nguyễn Văn A", "NXB Giáo dục", 2020, 150, "Giáo trình toán học lớp 1", 20);
        Education_3 book17 = new Education_3("Ngữ văn cơ bản", "Trần Thị B", "NXB Giáo dục Quốc gia", 2019, 180, "Giáo trình ngữ văn lớp 2", 22);
        Education_3 book18 = new Education_3("Lịch sử Việt Nam", "Phạm Văn C", "NXB Giáo dục và Thanh niên", 2018, 200, "Giáo trình lịch sử lớp 4", 25);
        Education_3 book19 = new Education_3("Địa lý thế giới", "Hoàng Thị D", "NXB Đại học Quốc gia", 2017, 220, "Giáo trình địa lý lớp 6", 28);
        Education_3 book20 = new Education_3("Vật lý căn bản", "Lê Văn E", "NXB Giáo dục phổ thông", 2016, 250, "Giáo trình vật lý lớp 8", 30);
        Education_3 book21 = new Education_3("Hóa học hữu cơ", "Nguyễn Thị F", "NXB Giáo dục Việt Nam", 2015, 280, "Giáo trình hóa học lớp 10", 32);
        Education_3 book22 = new Education_3("Tiếng Anh giao tiếp", "Trần Văn G", "NXB Thanh Niên", 2014, 300, "Giáo trình tiếng Anh lớp 3", 35);
        Education_3 book23 = new Education_3("Tin học văn phòng", "Lê Thị H", "NXB Giáo dục Quốc dân", 2013, 270, "Giáo trình tin học văn phòng lớp 12", 33);
        Education_3 book24 = new Education_3("Sinh học phân tử", "Phan Văn I", "NXB Giáo dục và Đào tạo", 2012, 290, "Giáo trình sinh học lớp 11", 31);
        Education_3 book25 = new Education_3("Ngôn ngữ học hợp âm", "Nguyễn Thị K", "NXB Giáo dục phổ thông và Đào tạo", 2011, 310, "Giáo trình ngôn ngữ học lớp 9", 34);
        Education_3 book26 = new Education_3("Lịch sử thế giới", "Trần Văn L", "NXB Giáo dục và Đào tạo Việt Nam", 2010, 320, "Giáo trình lịch sử lớp 8", 36);
        Education_3 book27 = new Education_3("Địa lý Việt Nam", "Phạm Thị M", "NXB Giáo dục quốc gia Hà Nội", 2009, 330, "Giáo trình địa lý lớp 5", 37);
        Education_3 book28 = new Education_3("Hóa học vô cơ", "Lê Văn N", "NXB Giáo dục Việt Nam và Thanh thiếu niên", 2008, 340, "Giáo trình hóa học lớp 9", 38);
        Education_3 book29 = new Education_3("Tiếng Anh kỹ năng", "Trần Thị O", "NXB Thanh niên Hồ Chí Minh", 2007, 350, "Giáo trình tiếng Anh lớp 6", 39);
        Education_3 book30 = new Education_3("Tin học đồ họa", "Nguyễn Văn P", "NXB Giáo dục và Thanh niên Việt Nam", 2006, 360, "Giáo trình tin học đồ họa lớp 8", 40);
        Education_3 book31 = new Education_3("Sinh học sinh thái", "Lê Thị Q", "NXB Giáo dục phổ thông Hà Nội", 2005, 370, "Giáo trình sinh học lớp 7", 41);
        Education_3 book32 = new Education_3("Ngôn ngữ học ám sát", "Nguyễn Văn R", "NXB Giáo dục phổ thông và Đào tạo Hà Nội", 2004, 380, "Giáo trình ngôn ngữ học lớp 7", 42);
        Education_3 book33 = new Education_3("Lịch sử Việt Nam cận đại", "Trần Thị S", "NXB Giáo dục quốc gia Việt Nam", 2003, 390, "Giáo trình lịch sử lớp 10", 43);
        Education_3 book34 = new Education_3("Địa lý thế giới địa cầu", "Phạm Văn T", "NXB Giáo dục Việt Nam và Thanh thiếu niên", 2002, 400, "Giáo trình địa lý lớp 7", 44);
        Education_3 book35 = new Education_3("Hóa học hữu cơ vô cơ", "Hoàng Thị U", "NXB Giáo dục phổ thông Hà Nội", 2001, 410, "Giáo trình hóa học lớp 11", 45);

        t.put("bk1", book1);
        t.put("bk2", book2);
        t.put("bk3", book3);
        t.put("bk4", book4);
        t.put("bk5", book5);
        t.put("bk6", book6);
        t.put("bk7", book7);
        t.put("bk8", book8);
        t.put("bk9", book9);
        t.put("bk10", book10);
        t.put("bk11", book11);
        t.put("bk12", book12);
        t.put("bk13", book13);
        t.put("bk14", book14);
        t.put("bk15", book15);
        t.put("bk16", book16);
        t.put("bk17", book17);
        t.put("bk18", book18);
        t.put("bk19", book19);
        t.put("bk20", book20);
        t.put("bk21", book21);
        t.put("bk22", book22);
        t.put("bk23", book23);
        t.put("bk24", book24);
        t.put("bk25", book25);
        t.put("bk26", book26);
        t.put("bk27", book27);
        t.put("bk28", book28);
        t.put("bk29", book29);
        t.put("bk30", book30);
        t.put("bk31", book31);
        t.put("bk32", book32);
        t.put("bk33", book33);
        t.put("bk34", book34);
        t.put("bk35", book35);
        t.put("bk36", book36);
        t.put("bk37", book37);
        t.put("bk38", book38);
        t.put("bk39", book39);
        t.put("bk40", book40);
        t.put("bk41", book41);
        t.put("bk42", book42);
        t.put("bk43", book43);
        t.put("bk44", book44);
        t.put("bk45", book45);
        t.put("bk46", book46);
        t.put("bk47", book47);
        t.put("bk48", book48);
        t.put("bk49", book49);
        t.put("bk50", book50);

    }

    public HashMap<String, Object> getDataListBook() {
        return t;
    }

    public void display() {
        for (String s : t.keySet()) {
            System.out.println("------------");
            System.out.println(s);
        }
    }

    public void DisplayListNameBook() {

        ListBooks obj = new ListBooks();
        for (int i = 1; i <= obj.getDataListBook().size(); i++) {
            Books save = (Books) obj.getDataListBook().get("bk" + i);
            String namebook = save.getTitle();
            System.out.println("Mã Code : bk"+i +" |"+" Name Book :"+namebook);
        }

    }

}
